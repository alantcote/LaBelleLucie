...
public class FanView extends AnchorPane implements View {
	...
	/**
	 * The number of cards in a fan that can be displayed with minimum overlap, this
	 * is used to calculate the size of the view. When the actual number of cards in
	 * the fan is larger than this number, the overlap is increased, to permit all
	 * of the cards to be displayed, though those not on top have smaller visible
	 * portions.
	 */
	 ...
	 */
	public FanView(CardViewFactory cardViewFactory, double fanOffset, Fan model) {
		super();

		Dimension2D cardSize = cardViewFactory.getDimensions();
		double minHeight = getHeight(cardSize) + (MARGIN * 2);
		double minWidth = getWidth(cardSize) + (MARGIN * 2) + (fanOffset * (MAX_FAN_SIZE - 1));
			...
			if (node instanceof CardView) {
				Thread thread = new Thread() {
					...
					public void run() {
						...
						ColorInput colorInput = new ColorInput(bounds.getMinX(), bounds.getMinY(), bounds.getWidth(),
								bounds.getHeight(), Color.WHITE);
	...
	/**
	 * Anchor a given view at a given distance from the left-hand edge of this view.
	 ...
	 * @param indent the distance at which the given view is to be placed from the
	 *               left-hand edge of this view.
	 */
	...
	/**
	 * Anchor a given view at a given distance from the top edge of this view. The
	 * method that <code>AnchorPane</code> provides for this purpose is
	 * <code>static</code>, and therefore unmockable. This method provides a means
	 * of mocking out the behavior, for unit testing.
	 * 
	 * @param view   the view to be anchored.
	...
	/**
	 * Install a given tooltip on a node. This method exists to provide a means of
	 * mocking calls to <code>Tooltip</code>'s <code>install() static</code> method.
	 * 
	 * @param node    the node.
	 * @param tooltip the tooltip.
	 */
	...
	protected void layoutChildren() {
		/**
		 * If getChildren().size() <= MAX_FAN_SIZE, use fanOffset. Else use (fanOffset *
		 * (MAX_FAN_SIZE - 1)) / (getChildren().size() - 1).
		 */
		int layer = 0;
		ObservableList<Node> kids = getChildren();
	...
	/**
	 * Create a new <code>FanBinding</code> object. This method is included here to
	 * enable mocking new object creation.
	 * 
	...
	/**
	 * Create a new object. This method is provided to enable mocking the behavior.
	 * 
	...
	/**
	 * Create a new <code>InputHandlerSupport</code> object. This method is included
	 * here to enable mocking new object creation.
	 * 
	 * @param supported the supported object.
	 * @return the new object.
	 */
	...
	/**
	 * Set the id of this node, to enable CSS to support it. The method that
	 * <code>Node</code> provides for this purpose is final, so it cannot be mocked.
	 * This method is used to wrap that method, to facilitate unit testing of this
	 * class.
	 * 
	 * @param id the id.
	 */
