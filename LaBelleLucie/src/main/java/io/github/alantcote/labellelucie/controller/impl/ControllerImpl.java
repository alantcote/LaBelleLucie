...
import io.github.alantcote.labellelucie.controller.impl.undoableop.MoveCardTableauToTableauOp;
...
public class ControllerImpl implements Controller {
	...
	/**
	 * Move a card from a given index in a given fan to the top (end) of the fan.
	 * 
	...
	/**
	 * Move the top card from a given foundation fan to the top of a given tableau
	 * fan. This method's primary use is in support of undo.
	 * 
	 * @param srcFanIndex  the foundation fan index.
	 * @param destFanIndex the tableau fan index.
	 */
	public void moveTopCardFoundationToTableau(int srcFanIndex, int destFanIndex) {
		Fan[] tableauFan = model.getTableau();
		Fan[] foundationFan = model.getFoundation();
		Card card = foundationFan[srcFanIndex].remove(foundationFan[srcFanIndex].size() - 1);

		tableauFan[destFanIndex].add(card);
	}

	/**
	 * Move the top card from a given tableau fan to the top of a given foundation
	 * fan.
	 * 
	 * @param srcFanIndex  the tableau fan index.
	 * @param destFanIndex the foundation fan index.
	 */
	public void moveTopCardTableauToFoundation(int srcFanIndex, int destFanIndex) {
		Fan[] tableauFan = model.getTableau();
		Fan[] foundationFan = model.getFoundation();
		Card card = tableauFan[srcFanIndex].remove(tableauFan[srcFanIndex].size() - 1);
...
//			log.info("moves = " + moves);
			...
			for (Move move : moves) {
				if (sourceFanIndex == move.getSrcFanIndex()) {
					...
					case TABLEAU_TO_FOUNDATION:
						moveCardToFoundation(sourceFanIndex, move.getDestFanIndex());
						break;
					case TABLEAU_TO_TABLEAU:
						moveCardToTableau(sourceFanIndex, move.getDestFanIndex());
						break;
					default:
						break;
					}
	...
	/**
	 * Move the top card from a given tableau fan to atop a given foundation fan.
	 * 
	 * @param srcFanIndex  the source tableau fan index.
	...
	protected void moveCardToFoundation(int srcFanIndex, int destFanIndex) {
		MoveCardTableauToFoundationOp op = newMoveCardTableauToFoundationOp(srcFanIndex, destFanIndex);
	...
	/**
	 * Move the top card from a given tableau fan to atop another given tableau fan.
	 * 
	 * @param srcFanIndex  the source tableau fan index.
	 * @param destFanIndex the destination tableau fan index.
	 */
	protected void moveCardToTableau(int srcFanIndex, int destFanIndex) {
		MoveCardTableauToTableauOp op = newMoveCardTableauToTableauOp(srcFanIndex, destFanIndex);
	...
	/**
	 * Create a new operation to move a card from the top of a tableau fan to the
	 * top of a foundation fan.
	 * 
	 * @param srcFanIndex  the index of the tableau fan.
	 * @param destFanIndex the index of the foundation fan.
	 * @return the new operation.
	 */
	protected MoveCardTableauToFoundationOp newMoveCardTableauToFoundationOp(int srcFanIndex, int destFanIndex) {
		return new MoveCardTableauToFoundationOp(this, srcFanIndex, destFanIndex);
	}

	/**
	 * Create a new operation to move a card from the top of one tableau fan to the
	 * top of another foundation fan.
	 * 
	 * @param srcFanIndex  the index of the source tableau fan.
	 * @param destFanIndex the index of the destination tableau fan.
	 * @return the new operation.
	 */
	protected MoveCardTableauToTableauOp newMoveCardTableauToTableauOp(int srcFanIndex, int destFanIndex) {
