...
/**
 * A solution for Project Euler Problem 22.
 * The problem statement is
 * <a href="http://projecteuler.net/problem=22">here</a>.
 * @author Al Cote'
 */
public class Problem22 {
	public static final char LETTER_BASE = 'A' - 1;
	public static final String[] RAW_NAMES = { "MARY", "PATRICIA", "LINDA",
	...
	public static void main(String[] args) {
		int nameCount = RAW_NAMES.length;
		String[] sortedNames = Arrays.copyOf(RAW_NAMES, nameCount);
		int sum = 0;
		
		Arrays.sort(sortedNames);
		
		for (int alphaPos = 0; alphaPos < nameCount; ++alphaPos) {
			String name = sortedNames[alphaPos];
			int oldSum = sum;
			int letterSum = 0;
			int product;
			...
			for (int letterPos = 0; letterPos < name.length();
					++letterPos) {
				char letter = name.charAt(letterPos);
				int number = letter - LETTER_BASE;
				
				letterSum += number;
			}
			
			product = letterSum * (alphaPos + 1);
			sum += product;
			
			if (sum < oldSum) {
				System.err.println(
						"sum overflowed from " + oldSum + " to " + sum);
				System.exit(1);
			}
		}
